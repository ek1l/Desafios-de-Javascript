...
switch (escolherConversao) {
  case "1":
    let escolherTemperatura = prompt(
      `Agora digite a temperatura Celsius  \n que será convertida para Farenheit`
    );
    alert(
      `A conversão de ${escolherTemperatura} Celsius para Farenheit é: ${Math.floor(
        converterTemperatura(escolherTemperatura)
      )}`
    );
    break;
  case "2":
    let escolherTemperaturaFarenheit = prompt(
      `Agora digite a temperatura Farenheit  \n que será convertida para Celsius`
    );
    alert(
      `A conversão de ${escolherTemperaturaFarenheit} Farenheit para Celsius é: ${Math.floor(
        converterTemperatura2(escolherTemperaturaFarenheit)
      )}`
    );
    break;
  default:
    alert(
      `Ops, algo deu errado, tente reiniciar a página. \n Digite 1 ou 2 para prosseguir :(`
    );
}
